...
def build_system_prompt(
    ...
    """
    Builds the complete system prompt for Azure SQL / T-SQL generation.
    Optional blocks are injected between dialect rules and the schema section.

    semantic_block      — business concept definitions (from semantic_layer.py)
    join_hints_block    — pre-resolved JOIN paths (from join_graph.py)
    conversation_block  — recent conversation history (from conversation_context.py)
    """
    extra_sections = ""
...
9. When BUSINESS DEFINITIONS are provided, follow them EXACTLY — do not substitute
