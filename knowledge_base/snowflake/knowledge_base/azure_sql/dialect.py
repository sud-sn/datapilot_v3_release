...
def build_system_prompt(
    ...
    """
    Builds the complete Azure SQL / T-SQL SQL generation prompt.
...
Rule 11: Write ONLY valid T-SQL syntax. Never use PostgreSQL or Snowflake constructs.
Rule 12: Wrap all object names in square brackets: [schema].[table].[column].
Rule 13: Boolean values are 1 / 0 (BIT type). Never write TRUE or FALSE.
...
def build_schema_context_string(schema_context) -> str:
    """
    Returns the EXACT COLUMN REGISTRY + detailed schema block for Azure SQL.
    Uses square bracket notation in the detail section for T-SQL compatibility.
    """
    from knowledge_base.schema_enforcement import (
        build_exact_column_registry, SQL_GENERATION_RULES
    )

    # Registry uses plain names for LLM lookup
    registry = build_exact_column_registry(schema_context)

    # Detail block uses T-SQL bracket notation
    lines: list[str] = [
