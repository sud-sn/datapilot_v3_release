...
class KnowledgeStore:
    ...
    def get_column_notes(self, source_id: str) -> dict[str, dict[str, str]]:
        """
        Parse column-level notes (including user-defined NEEDS CONTEXT resolutions)
        from the KB markdown for a source.

        Returns: {table_name: {column_name: note_text}}

        These notes are injected into the SQL generation prompt so the LLM
        follows user-defined column semantics for every query.
        """
        md = self.load_markdown(source_id)
        if not md:
            return {}
        return parse_column_notes(md)
...
def parse_column_notes(markdown: str) -> dict[str, dict[str, str]]:
    """
    Parse user-defined column notes from all table sections.

    Returns: {table_name: {column_name: note_text}}

    This captures two kinds of notes:
      1. Regular notes the LLM wrote explaining a column
      2. User-edited notes where the user replaced ⚠️ NEEDS CONTEXT
         with plain English like "Use this as the doctor's name"

    The result is injected into the SQL generation prompt so the LLM
    follows user-defined column semantics.

    Example markdown column row:
      | FULL_NAME | VARCHAR | Prescriber full name | Use this as the doctor's name |
      | STATUS_CODE | VARCHAR | Status indicator | ⚠️ NEEDS CONTEXT |
      | RX_DATE | DATE | Prescription date | Date when prescription was written |
    """
    ...
    for table_match in re.finditer(
        ...
        # Find the Columns table within this table section
        ...
        for line in col_section.group(1).split("\n"):
            ...
            # Skip header row
            if col_name.lower() in ("column", "[exact column name]"):
                continue
            if not col_name or col_name.startswith("["):
                continue

            # Skip unfilled NEEDS CONTEXT flags
            if "⚠️ NEEDS CONTEXT" in note and note.strip() == "⚠️ NEEDS CONTEXT":
                continue

            # Strip the flag prefix if user added context after it
            # e.g. "⚠️ NEEDS CONTEXT — Use this as the doctor's name"
            if "⚠️ NEEDS CONTEXT" in note:
                # User added their definition after the flag
                after_flag = re.sub(r"⚠️ NEEDS CONTEXT\s*[-—–]?\s*", "", note).strip()
                if after_flag:
                    note = after_flag

            if note and col_name:
                col_notes[col_name] = note

        if col_notes:
            result[table_name] = col_notes

    logger.debug(
        "Parsed column notes: %d tables, %d total annotated columns",
        len(result),
