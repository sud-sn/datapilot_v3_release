...
class NLPToSQLPipeline:
    ...
    concept_registry:
        ...
        Pass None to use the global defaults from semantic_layer.py.
        Pass [] to use no concept enrichment (raw schema only).
        Pass a list to use concepts from the knowledge store (recommended).
    ...
    def __init__(
        ...
        logger.info(
            "Pipeline ready: source=%s | kb=%s | concepts=%s",
    ...
    def refresh_concepts(self, concepts: list[SemanticConcept]) -> None:
        ...
        Called after the user edits and saves the KB markdown.
        """
